...
4) Caves not done because this system has not installed caves_system :)
5) new Grass texture of terrain
DONE 6) Axe model
7) Fog darkens/changes colour if we descend height
DONE 8) Trees not done because this system has not installed Tree_system :)
...
DONE near future) axe have bugs!
not so DONE future yet) (very REAR) Mining! 
"""
...
# distance of build Yuvraj, thanks!)
build_distance = 3
...
currentSubset = 0
numSubCubes = 64
numSubsets = 420 #how many combined into a megaset?
theta = 0
rad = 0
